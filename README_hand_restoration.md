...
The formal configuration trains at 512x512 for ten dataset epochs with a micro-batch of 4 and two-way gradient accumulation (effective batch 8). At the documented 150 frames per clip and complete right-hand availability, that is 2,400 train samples, 300 optimizer steps per epoch, and 3,000 optimizer steps total. Every indexed sample is visited once per epoch and therefore ten times over the run, while diffusion noise and timestep are resampled on every visit. The trainer derives the actual total from the indexed sample count, runs fixed-order, fixed-seed validation exactly once per indexed epoch, and records configuration, split, Git commit, GPU, dependency versions, losses, steps, samples seen, and timing in the experiment directory. Resume remains available with `--resume PATH_TO_CHECKPOINT`.
